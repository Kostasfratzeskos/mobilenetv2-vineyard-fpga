...
**Επαλήθευση.** Το `accel_tb` τρέχει τις τρεις πρώτες εντολές του **πραγματικού**
προγράμματος από άκρη σε άκρη, με το testbench στον ρόλο του DMA:

| op | opcode | layer | στοιχεία | golden |
|---:|:------|:------|---------:|:-------|
| 0 | STEM | features.0.0 | 401.408 | 001 |
| 1 | DW | features.1.conv.0.0 | 401.408 | 002 |
| 2 | PW | features.1.conv.1 | 200.704 | 003 |

Σύνολο **1.003.520 στοιχεία bit-exact**, αλυσιδωτά — το op 1 διαβάζει από το
`base=12544` που του έδωσε ο static allocator, χωρίς κανείς να του το πει εκτός από
το instruction word.

**Τι βρήκε που καμία μονάδα δεν μπορούσε να βρει.** Δύο bugs στο `busy` handshake
(DD-017): μία τρύπα ενός κύκλου σε τέσσερις feeders, και το `pw_feeder` που εξέθετε
το `busy` του address generator του — ένα ολόκληρο pipeline depth νωρίς, που κόστιζε
τα δύο τελευταία pixels κάθε pointwise layer. Και τα δύο χρειάζονταν **δεύτερο op**
για να κάνουν ζημιά, γι' αυτό και κανένα standalone testbench δεν τα έπιανε.

**Επόμενο βήμα:** DMA πίσω από το `ld_req`/`ld_done`, και ο compiler να εκπέμπει τις
παραμέτρους των RES/GAP (τώρα βγαίνουν με `pchan=0`, οπότε το `top_seq` παίρνει τον
δρόμο `no_load` και δεν ζητά ποτέ το `(m0, shift)` που χρειάζονται). Μετά σύνθεση
στο Vivado.
