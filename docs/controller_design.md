...
### 7.7 Datapath top — ΕΤΟΙΜΟ (2026-09-13)

Το `accel_top.v` είναι το module που κάνει το μοίρασμα **πραγματικό**. Ως τώρα το
`out_stage` ήταν κοινό ως *module* αλλά κάθε feeder είχε δικό του *instance* — δηλαδή
η οικονομία των DSP για την οποία φτιάχτηκε δεν είχε εισπραχθεί ποτέ. Τώρα υπάρχει
**ένα** `out_stage`, **μία** `act_buffer`, **ένα** `img_buffer` και ένα σύνολο
παραμέτρων, και ο opcode δείχνει ποιος feeder τα οδηγεί:

```
op_code --+--> ποιος feeder παίρνει op_start
          +--> ποιανού os_acc οδηγεί το κοινό out_stage
          +--> ποιανού a_* οδηγεί το read port της δεξαμενής
          +--> ποιανού aw_* οδηγεί το write port
```

Το `os_q` γυρίζει σε **όλους** τους feeders χωρίς gating: ένας αδρανής feeder θα
requantize ό,τι βρει στο bus και θα σηκώσει το δικό του `aw_en`, αλλά η εγγραφή του
δεν φτάνει ποτέ στη δεξαμενή επειδή ο mux διαλέγει με βάση τον opcode. Η απομόνωση
υπάρχει ήδη· το gating θα ήταν διπλό κόστος.

**Τι μετακινήθηκε στο instruction word.** Τα `act` και `relu6_qmax` ήταν καρφωμένα
μέσα σε τρεις feeders (res/gap → ACT_NONE, stem → ACT_RELU6). Με ένα κοινό stage
έρχονται πλέον από την εντολή, άρα δεν είναι πια ιδιότητες του RTL αλλά **υποχρεώσεις
του `gen_program.py`**. Ο compiler ήδη βγάζει ακριβώς αυτές τις τιμές και το
`accel_tb` το ελέγχει απέναντι στο πραγματικό πρόγραμμα αντί να το εμπιστεύεται.

**Επαλήθευση — ολόκληρο το δίκτυο.** Το `accel_tb` τρέχει και τις **64 εντολές**
του πραγματικού προγράμματος από άκρη σε άκρη — μία εικόνα μέσα, τέσσερα logits
έξω — και συγκρίνει **κάθε ενδιάμεσο tensor** με το software golden:

| | |
|:--|--:|
| ops | 64 |
| στοιχεία ελεγμένα | **6.895.780** |
| αποτέλεσμα | **bit-exact** |
| logits | −2278, 11929, −6915, −2884 (ακριβώς το golden) |
| κλάση | 1 = esca, όπως το software μοντέλο |

Το testbench παίζει τον ρόλο του DMA και — αυτό είναι το ενδιαφέρον — **δεν
χρειάζεται κανέναν πίνακα ανά layer**. Το `top_seq` δίνει `(wgt_off, wgt_bytes,
pchan)` και από αυτά προκύπτει και το πώς μοιράζονται τα bytes στα banks:

```
taps ανά κανάλι εξόδου = wgt_bytes / pchan
```

που είναι 27 για το stem (3×3×3), 9 για ένα depthwise, και για ένα pointwise
είναι το ίδιο το IC. Δηλαδή τα τρία νούμερα που λένε **πού** να διαβάσει λένε και
**πώς** να το τοποθετήσει. Το μόνο που ξέρει το testbench και δεν ξέρει το
hardware είναι με ποιο golden να συγκρίνει — και αυτό παράγεται από τον compiler
(`program_ops.svh`), διασταυρωμένο με το `golden_manifest.json`.

**Μετρημένοι κύκλοι.** Πρώτη πραγματική μέτρηση αντί για εκτίμηση:

| | κύκλοι | @250 MHz |
|:--|--:|--:|
| feeders busy | 951.833 | **3,807 ms** |
| πρόβλεψη μοντέλου (§3, TS=8) | — | 3,56 ms |

Απόκλιση **+7%** από το μοντέλο, που για ένα cycle model χωρίς pipeline fill/drain
είναι καλή συμφωνία. Προσοχή: τα 250 MHz παραμένουν **στόχος, όχι μέτρηση** —
τίποτα δεν έχει περάσει από σύνθεση ακόμα.

**Τι βρήκε που καμία μονάδα δεν μπορούσε να βρει.**

1. Δύο bugs στο `busy` handshake (DD-017): μία τρύπα ενός κύκλου σε τέσσερις
   feeders, και το `pw_feeder` που εξέθετε το `busy` του address generator του.
2. Τα RES_ADD και GAP έβγαιναν με `pchan=0`, οπότε το `top_seq` έπαιρνε τον δρόμο
   `no_load` και **δεν ζητούσε ποτέ** το `(m0, shift)` τους. Οι παράμετροι έμεναν
   αυτές του προηγούμενου layer.
3. Το GAP έπαιρνε `n_pix=1` (pixels **εξόδου**) ενώ το `gap_feeder` περιμένει τα
   pixels **εισόδου** — μέσο όρο σε ένα pixel αντί για 49. Το πεδίο σημαίνει
   «πόσες χωρικές θέσεις διατρέχει ο feeder», που συμπίπτει με την έξοδο για κάθε
   op εκτός από το global pooling.

Και τα τρία χρειάζονταν **δεύτερο op** ή **ολόκληρη την αλυσίδα** για να φανούν.

### 7.8 Σύνθεση — τα νούμερα παύουν να είναι εκτιμήσεις (2026-09-13)

Ό,τι έλεγαν ως τώρα τα docs για πόρους και συχνότητα ήταν αριθμητική σε
υπολογιστικό φύλλο. Το `scripts/run_synth.sh` συνθέτει το `accel_top`
out-of-context για xczu7ev-ffvc1156-2-e. Πλήρης ανάλυση στο **DD-018**.

| | εκτίμηση | **μετρημένο** | |
|:--|--:|--:|:--|
| DSP | 442 (26%) | **944** | 54,6% |
| CLB LUTs | — | **32.387** | 14,1% |
| Block RAM | — | **176** | 56,4% |
| URAM | — | **52** | 54,2% |
| Fmax | 250 MHz | **46,8 MHz** | WNS −17,4 ns |

**Το design χωράει.** Χρειάστηκαν δύο synthesis directives — `ram_style="ultra"`
στη δεξαμενή (12,85 Mbit, περισσότερα από όλη τη BRAM της συσκευής, και το
εργαλείο την έβαζε εκεί) και `use_dsp="yes"` στους MACs. Είναι attributes, άρα
το xsim τα αγνοεί και **και τα 31 testbenches μένουν έγκυρα ως έχουν**.

Τα 944 DSP έναντι 442: η εκτίμηση προϋπέθετε int8 packing (δύο MAC ανά DSP48E2,
WP486) που **δεν έχει υλοποιηθεί**. Χωρίς packing: 512 (pointwise) + 144
(depthwise) + 216 (stem) + requantize ≈ 944.

**Το timing είναι το πραγματικό εύρημα.** Η χειρότερη διαδρομή:
...
Αλυσίδα **27 DSP στη σειρά χωρίς κανένα register ενδιάμεσα** — τα 27 taps του
stem (3×3×3) αθροισμένα συνδυαστικά σε **έναν** κύκλο. Το `mac_lane` κάνει το
ίδιο με 16. Ένα DSP48E2 φτάνει την ονομαστική του ταχύτητα μόνο με τα εσωτερικά
του pipeline registers· αλυσιδωμένο συνδυαστικά είναι αργό.

Άρα τα **250 MHz δεν επιτυγχάνονται με constraint ή attribute** — θέλουν
**pipelining των MAC trees**. Αυτό δεν έγινε μαζί επειδή το latency είναι
συμβόλαιο: εξαρτώνται από αυτό οι drain counters κάθε feeder, κάθε προθεσμία
`busy` (DD-017) και η αναμενόμενη σειρά αποτελεσμάτων κάθε testbench. Είναι
δικό του build step, με κριτήριο να εξακολουθεί να περνά ο έλεγχος των 6.895.780
στοιχείων.

**Επόμενα βήματα:** (1) pipelining των MAC trees και νέα σύνθεση, (2) DMA πίσω
από το `ld_req`/`ld_done` (το `accel_tb` δείχνει ότι η διεπαφή αρκεί),
(3) place-and-route — το μόνο που δίνει πραγματικό Fmax.
